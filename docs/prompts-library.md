...
> 버전: **v1.25 (2026-06-06, Art Production Sprint 1 — Style Bible v1 WARM 톤 production reskin: §5.16 Character Pack 7 char × 4 emotion = 28 + §5.17 Environment Pack L1~L5 = 5 + §5.18 UI Theme 5 sheets + §5.19 VFX 5 sprites. Cool Sage/Royal Match anchor 톤 → BG Cream #FBF3E4 + Cocoa outline 3-4px + soft volumetric storybook. driver = gen_character_pack.py / gen_environment_pack.py / gen_ui_vfx_pack.py) — supersedes v1.24**

> **v1.25 변경 (2026-06-06, Art Production Sprint 1 — Style Bible v1 production reskin, supersedes v1.24)**: Style Bible v1 (`docs/art/style-bible-v1.md`) LOCK 이후 첫 production sprint. 4~5개 시각 언어 충돌을 **warm cozy premium-casual (Cooking Diary · Animal Restaurant · Travel Town, NOT Royal Match)** 단일 톤으로 production. **§5.15 guest avatar v1.24의 OLD Cool Sage/Royal Match 톤은 본 §5.16에서 Style Bible warm 톤으로 reskin (supersede)**. 핵심 톤 migration: Cool Sage #C8D5C0 bg → **BG Cream #FBF3E4** / slim outline 2-3px #2D1D14 → **Cocoa outline 3-4px #3A2A1E** / Royal Match saturated 80-90% → **muted warm 55-78%** / flat single-fill/1-cel → **soft volumetric 2-tone (base×0.85)** / chibi 1:1.7 → **storybook chibi 1:1.3~1.6** / light pink #FFCFCF blush → **warm peach #E89A7A @40%**. 4 신규 prompt set + 3 신규 driver: **(a) §5.16 Character Pack — 7 character × 4 emotion = 28** (5 친구/멘토 guests.csv + **mother_01/father_01 신규 bust avatar** [기존엔 R-01~06 reaction만, bust 0건]). silhouette+color identity 강제 (junho spiky/red·mina ponytail/yellow·riley wavy-freckle/blue·mrs_lee permed-glasses/mauve+beige·seoyeon long-straight/oat·mother round-bun/coral·father salt-pepper/tan — 흑백 실루엣+상의색 1 block으로 7명 구분 = Style Bible §4.1 silhouette test). emotion = Style Bible §4.5 ★ mapping (neutral=okay/★2~3, happy=good/★4, excited=excellent/★5, disappointed=bad/★1 — **subtle lowered-eyebrow frown, NOT crying/teardrop**). 2-pass (Phase B neutral generate → Phase C 3 emotion image edit, family IP lock — gen_guest_avatars 패턴). driver `tools/gen_character_pack.py` (STYLE_SUFFIX_CHARACTER + CHARACTERS 7 + EMOTIONS_PHASE_B/C inline). 출력 `assets-raw/character_pack_m2/{id}_{emotion}_v1.png`. **(b) §5.17 Environment Pack L1~L5 = 5** (cooking backdrop, BG-01~05 storefront와 다른 역할 — 요리하는 환경). L1 home_kitchen(cream+oak 소박) → L2 snack_shop(분식집 철판+멜라민) → L3 traditional_market(시장 wood+onggi+기와 silhouette) → L4 food_alley(먹자골목 밤 warm lantern bokeh) → L5 prestige_restaurant(한정식 walnut+brass+한지). **3-band parallax-ready** (FG counter / MID props / BG warm-gray silhouette 명도 분리 + Cocoa soft shadow, blur X = Travel Town depth). 캐릭터 비포함 (Godot 레이어 합성). driver `tools/gen_environment_pack.py`. 출력 `assets-raw/environment_pack_m2/{Lx}_{name}_v1.png`. **(c) §5.18 UI Theme 5 sheets** (card_frame/panel_frame/button_family/ribbon_family/icon_family — UI-01~07 reskin). transparent + matte warm + round corner(button 24/card 28/chip h/2) + Cocoa shadow + gold-brass 절제(ribbon만). **9-slice/procedural 권고: frame(card/panel/button)=procedural StyleBoxFlat 우선** (round-rect+단색+outline+shadow = corner_radius/bg_color/border/shadow_color로 100% 재현, crisp+recolor+memory 0), **icon/ribbon=art** (유기적 pictogram+gold-brass sheen은 procedural 비효율). **(d) §5.19 VFX 5 sprites** (steam/sparkle/oil_splash/cooking_glow/friendship_gain — VFX-01~05 reskin). transparent + **subtle only (Style Bible §8)** — 화면폭발/confetti/shake/neon 금지, warm tone(white steam/gold sparkle/persimmon glow)만, cool-tone VFX 금지. UI+VFX driver `tools/gen_ui_vfx_pack.py --group ui|vfx|all`. 출력 `assets-raw/ui_vfx_pack_m2/{id}_v1.png`. **main thread 실행**: P1 character가 gate — (1) `py tools/gen_character_pack.py --phase B --only junho` test → (2) `--phase B` 7 neutral → **silhouette test + Mrs Lee≠Mother 확인 + LOCK** → (3) `--phase C` 21 emotion. P2 `py tools/gen_environment_pack.py`, P3/P4 `py tools/gen_ui_vfx_pack.py --group ui` / `--group vfx`. 비용 medium 43장 ~$1.81 (reroll 포함 ~$2.7), ~20min 생성. 상세 = `docs/art/asset-production-sprint1.md`. **ChatGPT 약점 risk** (driver negative 강제): Cool Sage/cool-mint 잔재 누수 / Mrs Lee↔Mother confused ~30% / disappointed→crying ~25% / excited→Royal Match explosion or Looney Tunes ~25% / mina→anime girl ~30% / 6 char→Western cartoon ~35%(Riley 제외) / 천막→이탈리아 stripe / env depth 평면화 / UI glossy / VFX 과폭발 / text 베이킹. 누수 시 자연어 reroll "배경 warm cream #FBF3E4, Animal Restaurant cozy 톤으로 다시" (sref 대체). v1.24 §5.15 이하 전 section은 placeholder reference로 보존 (Style Bible 이전 anchor cluster는 historical record).
