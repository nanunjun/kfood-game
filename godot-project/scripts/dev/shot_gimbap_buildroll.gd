## shot_gimbap_buildroll.gd — Build=Roll setup + 김발 보이게 + 연속성 F5 verification (2026-06-14).
##
## 사용자 F5 비교 교정 증명:
##   1) build_initial   : mat + 김 + 밥 (재료 놓기 전) — **김발(bamboo mat)이 김보다 크게 보임**.
##   2) build_placed    : 4 재료를 **실제 drag**로 밥 lower-third에 안착 — 밥 위 색색 다발(floating X).
##   3) roll_initial    : Roll 초기 setup(roll_flat_setup painterly) + **김발 추가** — Build 최종과 동일.
##   4) roll_edge_lift  : 그 상태에서 하단 edge 들림 시작(roundness 진입).
##
...
var _out_dir: String = "user://gimbap_buildroll"
...
func _ready() -> void:
	...
	await _shot_build_initial()
	await _shot_build_placed()
	await _shot_roll("roll_initial", 0.06)     # 초기 setup(거의 평평) — Build 최종과 동일 시각.
	await _shot_roll("roll_edge_lift", 0.24)    # 하단 edge 들림 시작.
...
func _capture(name: String) -> void:
	...
	if img != null:
		img.save_png(out)
		print("[buildroll-shot] %s -> %s (%dx%d)" % [name, ProjectSettings.globalize_path(out),
			img.get_width(), img.get_height()])
	else:
		print("[buildroll-shot] %s — NO IMAGE (dummy renderer)" % name)


# 1) build_initial — mat + 김 + 밥, 재료 놓기 전. 김발이 김보다 크게 보이는지 검증.
func _shot_build_initial() -> void:
	var bg := _bg()
	...
	await _capture("build_initial")
	build.queue_free(); bg.queue_free()
	await get_tree().process_frame
	await get_tree().process_frame


# 2) build_placed — 4 재료를 실제 drag로 밥 lower-third에 안착(밥 위 색색 다발, floating X).
func _shot_build_placed() -> void:
	var bg := _bg()
	var build: Control = BuildScript.new()
	build.set_anchors_preset(Control.PRESET_FULL_RECT)
	get_tree().root.add_child(build)
	...
	if gesture != null and strips != null and sg != null:
		...
		var guide_y: float = sg.position.y + float(BuildScript.BUNDLE_TARGET_Y)   # gimbap_mat 밥 region lower-third.
	...
	print("[buildroll-shot] build_placed placed=%d balance=%.2f" % [
		int(build.get("_placed")),
		build.get_arrange_balance() if build.has_method("get_arrange_balance") else -1.0])
	await _capture("build_placed")
	build.queue_free(); bg.queue_free()
	await get_tree().process_frame
	await get_tree().process_frame


# 3/4) roll — 초기 setup(roundness 작음) + edge lift. 김발이 추가됐는지 + 연속성 검증.
