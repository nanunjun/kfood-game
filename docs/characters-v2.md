...
> 친밀도(개인 호감) = 시장 평판과 별개. 단골 할인·우선 입수·정보(`markets-v1 §3`). 대사 `[PLAYER_TYPE]` 분기 가능(예: 외국인 플레이어엔 "welcome to Korea" 톤, 교포엔 "welcome back" 톤).

## 5. 플레이어 분기 변수 (NPC 대사용)
- `[PLAYER_NAME]`: 선택/입력한 이름.
- `[PLAYER_TYPE]`: heritage_seeker(Mia)·newcomer(Alex)·local(Jin)·returnee(Sora)·traveler(Pat) — **5종**(은퇴자 옵션 제외). 일부 NPC 대사가 미세 분기(예: Riley는 newcomer/traveler에겐 "fellow foreigner" 톤, heritage엔 "reconnecting" 톤). 상세 `player-characters-v1`.

## 6. A/B 미결
- **[A/B] 미각 노출**: A=자연어/표정만(디폴트) / B=정성 힌트 아이콘. 디폴트 A.
- **[A/B] 평가자 정체 reveal**: A=L10(디폴트) / B=L7 암시. 디폴트 A.
- **[A/B] 다인 채점**: A=All-pass(디폴트) / B=가중평균. (`scoring-v2 §6`)
- **[A/B] NPC 플레이어 분기 깊이**: A=인사 톤만 미세 분기(디폴트, 경량) / B=서브 대사 다수 분기(번역량↑). 디폴트 A.
