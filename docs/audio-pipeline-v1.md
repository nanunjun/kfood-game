...
## 1.5 손맛 layering 패스 (정렬 후 필수)
정렬·정규화 후 **layering 패스**를 거쳐야 "손맛" 완성(`sound-guide §0.5`, `phase1/production-priorities §1`).
- 판정별(Perfect/Good/Miss) 레이어 + 액션별(칼질/양념/끓음/플레이팅) 질감 레이어 + 완성 sting 3등급.
- 햅틱 트리거와 0ms 동기(Perfect=짧고 단단 / Miss=길고 부드러움).
- 양념별 SFX는 게이지 색과 1:1(고춧가루/간장/소금/설탕).
> layering은 디자인·믹스 작업(코드 무관). QC(§3)에서 판정별 레이어가 비트에 안 밀리는지 함께 점검.
