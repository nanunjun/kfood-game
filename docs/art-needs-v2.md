...
| 신규 메뉴 완성샷 | 29 | 확장 메뉴(`image_prompts_v2_menus`), 검정 컷아웃 |
| 보편 NPC 재작성 | 11 손님 + 평가자 5 | 글로벌 톤(`image_prompts_v2_characters`) |
> 브랜딩: 평가자·UI의 미식 모티프는 **황금 숟가락**(별 X, `branding-v1`).

### 5.6.1 양념 입력·다인 UI (외부 리뷰 보완)
| 슬롯 | 비고 |
|---|---|
| 양념 게이지 + 양념 노트 | 색깔별 게이지·양념통 노트(`image_prompts_v2_ui UI-12`, `scoring §1.5`) |
| 다인 분할 플레이팅 그리드 | 게스트별 그릇+고명 3슬롯(`UI-13`, `scoring §6.0`) |
> 오디오: 박자 정렬·QC 파이프라인 `audio-pipeline-v1.md`(아트 무관, 사운드 슬롯 정합용).

## 6. 생성 우선순위 (권장)
1. **캐릭터 15 × 3표정** (가장 눈에 띔, 메타의 얼굴)
2. **그릇 9 + 플레이팅/인벤토리 UI** (신규 페이즈 가시화)
3. **도구 프로/마스터 18** (어워드 동기)
4. **특산품·명품 재료** + 트로피 룸 UI
> 기존 음식 12·기본 재료·기본 도구는 재사용. 전 슬롯 동일 컷아웃 파이프라인(`cutout_bg.py`).
